...
app.get('/gall', (req, res) => {
    res.sendFile(path.join(__dirname, "/gall.html"));
});
app.get('/reg', (req, res) => {
...
app.post('/loginv', (req, res) => {
    ...
    const query1 = cur.find({ name: obj2.name }, function (err, data) {
        ...
        if (data.length < 1) {
            res.render('nr.ejs', { firstName: obj2.name, lastName: obj2.sur });
        }
        else {
            res.render('logv.ejs', { firstName: obj2.name, lastName: obj2.sur });
        }
    })
});
